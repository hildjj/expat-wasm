...
import wasmBuf from './expat.wasm.js'

const expat = await expatWasm({
  /**
   * @callback WasmCompleteCB
   * @param {WebAssembly.Instance} i
   */
  instantiateWasm (
    /** @type {WebAssembly.Imports} */ imports,
    /** @type {WasmCompleteCB} */ cb
  ) {
...
export class XmlParser extends EventEmitter {
  ...
  static XML_PARAM_ENTITY_PARSING_NEVER = 0;
  static XML_PARAM_ENTITY_PARSING_UNLESS_STANDALONE = 1;
  static XML_PARAM_ENTITY_PARSING_ALWAYS = 2;
   ...
   */
  static XML_SetParamEntityParsing(parser, code) {
    return expat.ccall(
      'XML_SetParamEntityParsing', 'number',
      ['number', 'number'],
      [parser, code]
    );
  }
